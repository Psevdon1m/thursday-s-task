...
contract FirstContract {
    ...
    event BookBought(address indexed addr, uint256 book);
    event BookSold(address indexed addr, uint256 book);
    ...
    constructor() public {
        ...
        books.push(Book({title: "Book A", price: 3000000000000000000}));
        books.push(Book({title: "Book B", price: 5000000000000000000}));
        books.push(Book({title: "Book C", price: 8000000000000000000}));
        books.push(Book({title: "Book D", price: 12000000000000000000}));
        books.push(Book({title: "Book E", price: 15000000000000000000}));
    }
    ...
    function _deposit(address addr, uint256 _amount) private {
    ...
    function buyBook(address addr, uint256 _bookIndex) public payable {
        ...
        emit BookBought(addr, _bookIndex);
    }
    ...
    function sellBook(address payable addr, uint256 _bookIndex) public {
        ...
        _removeBook(addr, bookToRemove);

        person.balance += books[_bookIndex].price;
        _sendFunds(addr, person.balance);

        emit BookSold(addr, _bookIndex);
    }

    function _removeBook(address addr, uint256 index)
        private
    ...
    function _sendFunds(address payable addr, uint256 amount) private {
        Person storage person = persons[addr];
        person.balance -= amount;
        addr.transfer(amount);

        emit Withdrawal(addr, amount);
    }
}
